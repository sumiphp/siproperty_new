...
if (!function_exists('sitedetails')) {
	function sitedetails()
	{
		...
		$sitedetails = array(
			...
			'shippingamountusd'=>$site['shippingamountusd'],
			'vatper'=>$site['vat'],
			'shipmsg'=>$site['shipmsg'],
			'checkoutmsg'=>$site['checkoutmsg'],
			'sitename' => $CompanyName,
			'site_shortname' => ((!empty($site) && isset($site['site_short_name']) && (trim($site['site_short_name']) != ''))? $site['site_short_name'] : $CompanyName),

			'logo'    => $siteLogo,
			'mobile_logo' => site_url().'assets/uploads/site/'.$site['site_mobile_logo'],
			...
			'site_linkldn' => ((!empty($site) && isset($site['site_linkldn']) && (trim($site['site_linkldn']) != ''))? (($site['site_has_linkldn'] == 1)? $site['site_linkldn'] : '') : 'https://www.linkldn.com'),
		);
		return $sitedetails;
	}
}
